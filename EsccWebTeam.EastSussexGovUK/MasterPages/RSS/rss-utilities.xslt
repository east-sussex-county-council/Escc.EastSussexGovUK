...
>
  ...
  <xsl:template name="HtmlFormatting">
    ...
    <xsl:choose>
      <xsl:when test="contains($CurrentUrl, 'eastsussex.gov.uk')">
        <xsl:text disable-output-escaping="yes">&lt;?xml-stylesheet  type="text/xsl" href="https://www.eastsussex.gov.uk/masterpages/rss/display-as-html-v2.xslt" ?&gt;
          &lt;?xml-stylesheet  type="text/css" href="https://www.eastsussex.gov.uk/escc.eastsussexgovuk/css/rssfeed.cssx" ?&gt;</xsl:text>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="ChannelHeader">
    ...
    <image>
      ...
      <url>https://www.eastsussex.gov.uk/masterpages/rss/escc-logo.gif</url>
      <width>90</width>
      <height>65</height>
      <link>
        <xsl:value-of select="$HtmlVersionUrl"/>
