...
-->        
        <xsl:choose>
            ...
            <!--
                string-length(normalize-space(translate(translate($time-string,'+',''),'-',''))) &lt; 9
                and
                string-length(translate($time-string,':','')) &lt; string-length($time-string)
                ...
                <xsl:when test="(string-length(normalize-space(translate($time-string,'+','')))+1) = string-length(normalize-space($time-string)) ">
                    ...
                    <xsl:if test="string-length(normalize-space(substring-before(translate($time-string,':',''),'+'))) &gt; 0">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="string-length(normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))) = 1">
                                <xsl:text>0</xsl:text>
                                <xsl:value-of select="normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                                <xsl:text>0000</xsl:text>                                
                            </xsl:when>
                            <xsl:when test="string-length(normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))) = 2">
                                <xsl:value-of select="normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                                <xsl:text>0000</xsl:text>                                
                            </xsl:when>
                            <xsl:when test="string-length(normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))) = 3">
                                <xsl:text>0</xsl:text>
                                <xsl:value-of select="normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                                <xsl:text>00</xsl:text>                                
                            </xsl:when>
                            <xsl:when test="string-length(normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))) = 4">
                                <xsl:value-of select="normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                                <xsl:text>00</xsl:text>                                
                            </xsl:when>
                            <xsl:when test="string-length(normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))) = 5">
                                <xsl:text>0</xsl:text>
                                <xsl:value-of select="normalize-space(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                            </xsl:when>
                            <xsl:when test="string-length(substring-before(translate(translate($time-string,':',''),'-',''),'+')) = 6">
                                <xsl:value-of select="(substring-before(translate(translate($time-string,':',''),'-',''),'+'))"/>
                            </xsl:when>
                        </xsl:choose>
                        <xsl:text>+</xsl:text>
                    <!-- after the TZ -->
                    <xsl:choose>
                        <xsl:when test="string-length(normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))) = 1">
                            <xsl:text>0</xsl:text>
                            <xsl:value-of select="normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))"/>
                            <xsl:text>00</xsl:text>                                
                        </xsl:when>
                        <xsl:when test="string-length(normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))) = 2">
                            <xsl:value-of select="normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))"/>
                            <xsl:text>00</xsl:text>                                
                        </xsl:when>
                        <xsl:when test="string-length(normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))) = 3">
                            <xsl:text>0</xsl:text>
                            <xsl:value-of select="normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))"/>
                        </xsl:when>
                        <xsl:when test="string-length(normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))) = 4">
                            <xsl:value-of select="normalize-space(substring-after(translate(translate($time-string,':',''),'-',''),'+'))"/>
                        </xsl:when>
                    </xsl:choose>
