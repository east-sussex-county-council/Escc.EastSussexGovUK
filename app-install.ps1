...
robocopy $sourceFolder "$destinationFolder\$projectName" /S /PURGE /IF *.dll *.ico *.css *.js apple-*.png navigation.png pan-*.gif *.master share.ascx related.ascx choose.ashx default.aspx error*.ascx escc-logo.gif logo-large.gif display-as-html.xslt config.xslt calendar.aspx hCalendar.ashx hCalendar.xslt datetime.xsl mf-templates.xsl uri.xsl xhtml2vcal.xsl /XD aspnet_client obj Properties "Web References"
